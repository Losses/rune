...
pub trait RemoteDataSource: Send + Sync + Debug {
    ...
    /// for a specific table. This operation **must be transactional**.
    ///
    /// # Arguments
    /// * `table_name`: The name of the table.
    /// * `operations`: A vector of `SyncOperation`s to apply.
    /// * `client_node_id`: The node ID of the client initiating the changes.
    /// * `new_last_sync_hlc`: The HLC that the client has determined as the new sync point. The server
    ///   should store this against the client's node ID for this table upon successful commit.
    ///
    /// # Returns
    /// The HLC generated by the remote server for this transaction.
    ...
    FKR: ForeignKeyResolver + Send + Sync,
{
    ...
    match remote_apply_result {
        Ok(achieved_remote_hlc) => {
            // The new last_sync_hlc is the maximum of what we processed
            // and what the server returned as its transaction HLC.
...
pub(crate) mod tests {
    ...
    async fn insert_test_record(
        ...
        let created_ts_str = created_hlc.to_rfc3339();
        let updated_ts_str = updated_hlc.to_rfc3339();
