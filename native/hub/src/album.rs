...
pub async fn fetch_albums_group_summary_request(
    ...
    let entry = count_albums(&main_db)
        .await
        .with_context(|| "Failed to fetch albums groups summary")?;
...
pub async fn fetch_albums_groups_request(
    ...
    let entry = get_albums_groups(&main_db, request.group_titles)
        .await
...
pub async fn fetch_albums_by_ids_request(
    ...
    let items = get_albums_by_ids(&main_db, &request.ids)
        .await
        .with_context(|| "Failed to fetch albums by id")?;
...
pub async fn search_album_summary_request(
    ...
    let items = list_albums(&main_db, request.n.try_into().unwrap())
        .await
